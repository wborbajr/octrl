...
//	error_reporting(0);
	...
	$aData['os_antendimento'] = format_data($data_os['ATENDIMENTO']);

	// Não é usado
	$aData['os_responsavel_tecnico'] = format_data($data_os['NOME_TECNICO_RESP']);

	$aData['os_situacao'] = format_data($data_os['DESCRICAO']);

	$aData['os_data'] = amdTOdma($data_os['DT_OS']);
	...
	$aData['os_contato'] = format_data($data_cliente['os_contato']);
  ...
  //
	$aData['os_imun'] = format_data($data_cliente['INSC_MUNIC']);
	...
	$aData['os_bairro'] = format_data($data_cliente['END_BAIRRO']);
	$aData['os_cidade'] = format_data($data_cliente['CIDADE'] .'/'. $data_cliente['UF']);
	...
	// $aData['os_problema'] = utf8_encode( addslashes($data_os['DEFEITO']));
