jQuery(function($) {
      ...
      console.log("index.js");
  		...
  		$("#lastOSs").on('click', function(){
  			lastOSs();
  		})


  		// Imprime a OS
  		$("#id_os_vendedor").on('click', function(e){
  				var sOS = $(this).text();
  				print_os(sOS);
  		});


  		// Carrega a OS em um frame para ser impressa
  		...
  		function lastOSs(){
  			...
  			$.ajax({
  							...
  							success:function(data, textStatus, jqXHR){
  								var rtn = data.data;
  								console.log("retornou", rtn);

  								// Chamar a função que recupera as ultimas informações
  								$("#id_os_geral").html(rtn.geral);
  								$("#id_os_vendedor").html(rtn.vendedor);
  							},
  							error:function(e){
  								console.log(e);
  								$("#id_os_geral").html("--0--");
    ...
    $("#modal_os").on("show.bs.modal", function(e) {
      ...
      // $.ajax({
      //     type: "POST",
      //     url: 'comum/retirada.inc.php',
      //     dataType: 'json',
      //     async: true,
      //     data: {acao: "item_add", obj: oOS, itens: oItem},
      //     error: function(result){
      //       console.log('não deu boa',result);
      //       alert("ERRO!! Ítem não adicionado!!");
      //     },
      //     success: function(result){
      //       var aDados = result.data;
      //
