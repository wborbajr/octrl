<?php
  ...
  // echo $sAcao;
    ...
    case "glt_dash_resumo6":
      $aRtn = glt_dash_resumo6();
      break;
...
function cargaRegistros($sLocal){
    ...
    if ($sLocal == ''){
      return Array(code => -1, data => Array(), msg => "Localidade [$sLocal] não informada!");
    }
    ...
    // Não encontrado ou url vazia
    if ($data['glt07_url'] == ''){
      return Array(code => -1, data => Array(), msg => "Localidade [$sLocal] não encontrada!");
    }
    ...
    if (! file_exists($sFonte)){
      return Array(code => -1, data => Array(), msg => "Fonte de dados [$sFonte] não encontrada!");
    }
    ...
    foreach ($lines as $line_num => $line) {
      ...
      if (($line != '') && (strlen($line) == 80) && (substr($line, 0, 4) != 'Date')){
        ...
        $sql = "INSERT INTO `glt15_registros` (
                        ...
                        "'".substr($line, 18,  2)."',".
                        "'".substr($line, 21,  4)."',".
                        "'".htmlspecialchars($aRamal[glt10_atendente])."',".
                        "'".substr($line, 33,  5)."',".
                        "'".substr($line, 39,  8)."',".
                        "'".substr($line, 48, 20)."',".
                        "'".substr($line, 69,  1)."'".
    ...
    $aRtn['Update'] = $iQtd;
    $aRtn['Linhas'] = $aLinhas;
    return Array(code => $iQtd, data => $aRtn, msg => "cargaRegistros()");
}
...
function glt_dash_combos(){
    ...
    while ($data = mysqli_fetch_array($res))
      $aRtn[Datas][] = array( 'data'   =>  formataPeriodo($data[0]) );


    $aResultado = Array(code => 0, data => $aRtn, msg => "glt_dash_combos()");
...
function glt_dash_resumo2(){
  ...
  $sSql = "SELECT `glt07_sigla`, EXTRACT(YEAR_MONTH FROM `glt15_data`) as periodo, 
              `glt05_tipo`, glt05_desc,  count(`glt07_sigla`) as qtd
           FROM glt15_registros, glt05_tipos
           WHERE glt05_tipo = glt05_codigo
           AND (glt15_hora > '09:00:00' AND glt15_hora < '18:30:00')
           GROUP BY 1, 2, 3, 4
  ...
  while ($data = mysqli_fetch_object($res)) {
   ...
   return Array(code => $iQtd, data => $aRtn, msg => "glt_dash_resumo2()");
}
...
function glt_dash_resumo5($sCidade, $sPeriodo, $sRamal, $sTipo){
  ...
  while ($data = mysqli_fetch_object($res)) {
   ...
   return Array(code => $iQtd, data => $aRtn, msg => 'glt_dash_resumo5() - ' . $sSql);
}
...
//***
// Quantos Tipos de Ligações por localidade, periodo
function glt_dash_resumo6(){

  $sSql = "SELECT `glt07_sigla`, EXTRACT(YEAR_MONTH FROM `glt15_data`) as periodo, 
                  glt05_tipo, glt05_desc, COUNT(`glt05_tipo`) AS qtd
            FROM glt15_registros, glt05_tipos
            WHERE glt05_tipo = glt05_codigo
            AND (glt15_hora > '09:00:00' AND glt15_hora < '18:30:00')
            GROUP BY 1, 2, 3, 4
  ...
  while ($data = mysqli_fetch_object($res)) {
   ...
   return Array(code => $iQtd, data => $aRtn, msg => 'glt_dash_resumo6()');
}
...
function MesToNum($sData){
  ...
  $sRtn = '20' . substr($sData, -2, 2) . $aMes[ substr($sData, 0, 3) ];
